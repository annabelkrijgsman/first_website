            ...
            <select id="selectcategory" class="button" onchange="selectCategory();">
                <option value=''>Select Category</option>
                    ...
                    <?php
                        ...
                        //CATEGORY FROM DATABASE INTO SELECT FORM
                        if (isset($_GET["cat"])) {
                            $catname = $_GET["cat"];
                        }
                        else {
                        ...
                        if (isset($_POST['submitsearch'])) {
                            ...
                            $row = mysqli_fetch_assoc($result);
                                ...
                                foreach ($result as $row) {
                                    ...
                                    echo '<h4 style="color:green">Your search results</h4>';
                                    echo '<h3>' . $row["Title"] . '</h3>';
                                    echo '<p><small>' . $row["Date"] . '</small></p>';
                                    echo '<a href="readblog.php?id=' . $row["ID"] . '">Read article..</a>';
                                    echo '</div>';
                                }   
                        }
                    ...
                    <?php
                        ...
                        if ($resultcheck > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo '<p><small>' . $row["Date"] . '</small></p>';
                                echo '<a href="readblog.php?id=' . $row["ID"] . '">Read article..';
                                echo '</div><hr/>';
                            }
                        }
